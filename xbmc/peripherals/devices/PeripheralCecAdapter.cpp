  ...
  if (adapter->m_bIsReady)
  {
    ...
    case CEC_OPCODE_STANDBY:
      ...
      if (command->initiator == CECDEVICE_TV &&
          //(adapter->m_bPowerOffOnStandby == 1 || adapter->m_configuration.bShutdownOnStandby == 1) &&
          (!adapter->m_standbySent.IsValid() || CDateTime::GetCurrentDateTime() - adapter->m_standbySent > CDateTimeSpan(0, 0, 0, SCREENSAVER_TIMEOUT)))
      {
        ...
        else if (adapter->m_bShutdownOnStandby == 1)
...
void CPeripheralCecAdapter::SetConfigurationFromLibCEC(const CEC::libcec_configuration &config)
{
  ...
  //m_bUseTVMenuLanguage = config.bUseTVMenuLanguage;
  bChanged |= SetSetting("use_tv_menu_language", m_bUseTVMenuLanguage == 1);

  //m_configuration.bActivateSource = config.bActivateSource;
  bChanged |= SetSetting("activate_source", m_configuration.bActivateSource == 1);

  //m_bPowerOffScreensaver = config.bPowerOffScreensaver;
  bChanged |= SetSetting("cec_standby_screensaver", m_bPowerOffScreensaver == 1);

  //m_bPowerOnScreensaver = config.bPowerOnScreensaver;
  bChanged |= SetSetting("cec_wake_screensaver", m_bPowerOnScreensaver == 1);

  m_configuration.bPowerOffOnStandby = config.bPowerOffOnStandby;

  //m_bSendInactiveSource = config.bSendInactiveSource;
  bChanged |= SetSetting("send_inactive_source", m_bSendInactiveSource == 1);
  ...
  bChanged |= SetSetting("standby_pc_on_tv_standby",
             ...
             m_bShutdownOnStandby ? 13005 : 36028);
